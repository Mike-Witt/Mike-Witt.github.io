...
# Notes
#
#   May 2015: It really does appear (from the associated notebook) that
#             the Bloch sphere is the same thing as the 3D spin vector.
#
# TTD
#   - I'm ready to try rotations in 3D (physical real, like "l" precession)
#   - Add animation (movie) code so I can point stuff on vimeo.
#
# Status
#
...
# These are things (mostly imports) that it makes sense to have
# defined globally. My motive for putting these here so that they'll
# be available in the ipython shell when I do: "ipython -i" for this
# file. Or course now that these things are defined, I may not realize
# I need to include them in specific function.
...
#####################################################################
#                                                                   #
#   Subroutines (probably some of these should move to sglib)       #
#                                                                   #
#####################################################################
...
# draw_vec()
#
# I've got a "similar" function in sglib, so there may be issues
# if I "import *" from both libraries and try to use this.
#    
def draw_vec(ax, v, color='black', label=None, fudge=1.1, **args):
    from sglib_v0 import Arrow3D
...
# test_bloch_2()
# 
# Rotate the state vector around the "real plane" drawing
# the equivalent block vectors.
#
def test_bloch_2(num_frames=50, size=[12, 6]):
    import matplotlib.pyplot as pl
    from mpl_toolkits.mplot3d import Axes3D
    from numpy import cos, sin, pi
    ...
    bbit = vb_bloch(ax2, alpha, beta, bs_lim=.75)

    dtheta = 2*pi / num_frames
    for n in range(num_frames):
        theta += dtheta
        alpha = cos(theta); beta = sin(theta)
        rbit.update(theta, alpha, beta)
        bbit.update(theta, alpha, beta)
    return(fig)

# test_bloch_3()
# 
# Complex plan and Bloch sphere. Here I should be able to try
# physically realistic evolution functions.
#
def test_bloch_3(num_frames=50, size=[12.5, 6.5]):
    import matplotlib.pyplot as pl
    from mpl_toolkits.mplot3d import Axes3D
    from numpy import cos, sin, pi
